...
/** NEW — clean API: only accepts an object */
export function useScaffoldWriteContract<TContractName extends ContractName>(
  config: UseScaffoldWriteConfig<TContractName>,
): ScaffoldWriteContractReturnType<TContractName> {
  const { contractName, chainId, writeContractParams } = config;
  ...
  >(
    ...
    if (!deployedContractData) {
      notification.error("Target contract not deployed.");
      return;
    }
    if (!accountChain?.id) {
      notification.error("Please connect your wallet.");
    ...
    if (accountChain.id !== selectedNetwork.id) {
      notification.error(`Wrong network — switch to ${selectedNetwork.name}`);
      return;
    }
    ...
    try {
      ...
      return await writeTx(makeWrite, { blockConfirmations, onBlockConfirmation });
    } finally {
      setIsMining(false);
    }
  };
  ...
  /** ---------------------------
   *  writeContract (non async UI version)
   *  --------------------------- */
  const sendContractWriteTx = <
    TFunctionName extends ExtractAbiFunctionNames<ContractAbi<TContractName>, "nonpayable" | "payable">,
  >(
    ...
    if (accountChain.id !== selectedNetwork.id) {
      notification.error(`Switch to ${selectedNetwork.name}`);
      return;
    }
